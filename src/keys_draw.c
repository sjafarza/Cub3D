...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/07 07:30:05 by saray             #+#    #+#             */
/*   Updated: 2021/04/23 11:53:52 by saray            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_key_press(int keycode, t_recup *recup)
{
	if (keycode == FORWARD_W_Z)
		...
		ft_error(recup, "No errors! success!\n");
	return (1);
}
...
void	ft_draw_texture(t_recup *recup, int x, int y)
{
	...
	recup->t.texx = (int)(recup->t.wallx * (double) \
		recup->texture[recup->t.texdir].width);
	...
	{
		recup->t.texy = (int)recup->t.texpos & (recup->texture \
				[recup->t.texdir].height - 1);
		recup->t.texpos += recup->t.step;
		if (y < recup->ry && x < recup->rx)
			recup->data.addr[y * recup->data.line_length / \
				4 + x] = recup->texture[recup->t.texdir].\
				addr[recup->t.texy * recup->texture[recup->\
				t.texdir].line_length / 4 + recup->t.texx];
	}
}
